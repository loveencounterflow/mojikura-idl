...
(function() {
  ...
  this._get_next_token = function(me, mode) {
    ...
    R = me.tokens[me.idx];
    if (R == null) {
      this._err(me, me.idx - 1, "syntax error: premature end of source");
    }
    if (mode !== 'peek') {
      this._advance(me);
    }
    ...
    while (true) {
      ...
      switch (type = token.t) {
        case 'lbracket':
          ...
          continue;
        case 'rbracket':
          this._err(me, me.idx - 1, "syntax error: unexpected right bracket");
          break;
        case 'operator':
          if (advance) {
            if (!(token.a > 1)) {
              this._err(me, me.idx - 1, "syntax error: cannot bracket unary operator");
            }
            target = [token];
            while (true) {
              ...
              if (this._token_is_rbracket(next_token)) {
                if (!(target.length - 1 > token.a)) {
                  this._err(me, me.idx, "syntax error: too few constituents");
                }
                this._advance(me);
                break;
        ...
        case 'proxy':
          if ((type === 'solitaire') && (me.idx !== 1)) {
            this._err(me, me.idx - 1, "syntax error: cannot have a solitaire here");
          }
          if (R != null) {
            R.push(token);
          } else {
          ...
          break;
        default:
          this._err(me, me.idx - 1, "syntax error: illegal token " + (rpr(token.s)) + " (type " + (rpr(type)) + ")");
      }
      break;
    }
    return R;
