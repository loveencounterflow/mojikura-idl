...
(function() {
  var CND, O, alert, badge, debug, echo, help, info, log, rpr, urge, warn, whisper;
  ...
  this.NCR = Object.create(require('ncr'));

  this.NCR._input_default = 'xncr';

  this.NCR.jzr_as_uchr = function(glyph) {
  ...
  this.NCR.jzr_as_xncr = function(glyph) {
    var nfo;
  ...
  this._get_tokenlist = function(me) {
    ...
    R = [];
    chrs = this.NCR.chrs_from_text(me.source);
  ...
  this._get_tokentree = function(me) {
    ...
    if ((me.tokenlist.length === 1) && ((type = me.tokenlist[0].t) === 'component')) {
  ...
  this._new_token = function(me, lexeme, idx) {
    ...
    null
     */
    lexeme = this.NCR.jzr_as_uchr(lexeme);
  ...
  this._token_as_text = function(me, token, jzr_mode) {
    ...
    switch (jzr_mode) {
      case 'uchr':
        return this.NCR.jzr_as_uchr(token.s);
      case 'xncr':
        return this.NCR.jzr_as_xncr(token.s);
      default:
        throw new Error("expected 'uchr' or 'xncr' for JZR mode, got " + (rpr(jzr_mode)));
    }
    return null;
  ...
  this._tokentree_as_formula = function(me, tokentree, jzr_mode) {
    var R, element, has_brackets, i, left, len, mid, right;
  ...
  this.shake_tree = function() {
    throw new Error("not implemented for IDL dialect");
  };

  this._shake_tree = function() {
    throw new Error("not implemented for IDL dialect");
  };
