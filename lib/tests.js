...
(function() {
  ...
  if (module.parent == null) {
    include = ["sanity checks (grammar data)", "(IDL) parse simple formulas", "(IDL) reject bogus formulas", "(IDL) parse tree of simple formulas", "(IDLX) reject bogus formulas", "(IDLX) reject IDL operators with arity 3", "(IDLX) parse simple formulas", "(IDLX) parse extended formulas (plain)", "(IDLX) parse extended formulas (bracketed)", "(IDLX) reject bogus formulas (bracketed)", "(IDLX) reject bogus formulas (solitaires)", "(IDL) _tokentree_as_formula", "(IDLX) _tokentree_as_formula", "(IDLX) formula_from_source (1)", "(IDLX) formula_from_source (2)", "(IDLX) sexpr_from_source", "(IDLX) doubt mark"];
    this._prune();
    this._main();
