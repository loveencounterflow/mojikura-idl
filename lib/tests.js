...
(function() {
  var CND, IDL, IDLX, MKNCR, alert, badge, debug, demo_errors, echo, help, include, info, log, nice_text_rpr, ref, rpr, test, urge, warn, whisper,
  ...
  this["(IDLX) parse extended formulas (plain)"] = function(T) {
    var i, len, matcher, probe, probes_and_matchers, ref1, result;
    probes_and_matchers = [['≈㐀', ['≈', '㐀']], ['≈𠀎', ['≈', '𠀎']], ['≈𪜀', ['≈', '𪜀']], ['≈〇', ['≈', '〇']], ['●', '●'], ['〓', '〓'], ['⿱〓〓', ['⿱', '〓', '〓']], ['▽', '▽'], ['↻正', ['↻', '正']], ['↔≈匕', ['↔', ['≈', '匕']]], ['↔正', ['↔', '正']], ['⿱丶乂', ['⿱', '丶', '乂']], ['⿺走⿹◰口戈日', ['⿺', '走', ['⿹', ['◰', '口', '戈'], '日']]], ['≈匚', ['≈', '匚']], ['≈&jzr#xe174;', ['≈', '&jzr#xe174;']], ['≈非', ['≈', '非']], ['⿺走⿹◰口〓日', ['⿺', '走', ['⿹', ['◰', '口', '〓'], '日']]], ['⿻串⿰立&jzr#x1234;', ['⿻', '串', ['⿰', '立', '&jzr#x1234;']]], ["⿱丶⿵𠘨§", ["⿱", "丶", ["⿵", "𠘨", "§"]]]];
    for (i = 0, len = probes_and_matchers.length; i < len; i++) {
      ref1 = probes_and_matchers[i], probe = ref1[0], matcher = ref1[1];
      result = IDLX.parse(probe);
      urge(JSON.stringify([probe, result]));
  ...
  this["(IDLX) parse extended formulas (bracketed)"] = function(T) {
    var i, len, matcher, probe, probes_and_matchers, ref1, result;
    probes_and_matchers = [['(⿰亻聿式)', ['⿰', '亻', '聿', '式']], ['(⿱北㓁允)', ['⿱', '北', '㓁', '允']]];
    for (i = 0, len = probes_and_matchers.length; i < len; i++) {
      ref1 = probes_and_matchers[i], probe = ref1[0], matcher = ref1[1];
      result = IDLX.parse(probe);
      urge(JSON.stringify([probe, result]));
  ...
  if (module.parent == null) {
    include = ["sanity checks (private methods)", "sanity checks (grammar data)", "sanity checks (MKNCR)", "(IDL) parse simple formulas", "(IDL) reject bogus formulas", "(IDL) parse tree of simple formulas", "(IDLX) reject bogus formulas", "(IDLX) reject IDL operators with arity 3", "(IDLX) parse simple formulas", "(IDLX) parse extended formulas (plain)"];
    this._prune();
    this._main();
    ...
    debug('30201', IDLX.parse('⿻串⿰立&jzr#x1234;'));
    debug('30201', IDLX.parse("⿱丶⿵𠘨§"));
  }
