...
(function() {
  ...
  this["(IDLX) reject bogus formulas (bracketed)"] = function(T) {
    var error, error1, i, len, matcher, message, probe, probes_and_matchers, ref1, result;
    probes_and_matchers = [["(⿰亻聿)", "syntax error: too few constituents in [ (⿰亻聿 ✘ ) ✘  ]"], ["(⿰亻)", "syntax error: too few constituents in [ (⿰亻 ✘ ) ✘  ]"], ["(⿰亻)聿", "syntax error: too few constituents in [ (⿰亻 ✘ ) ✘ 聿 ]"], ["(≈北㓁)", "syntax error: cannot bracket unary operator [ ( ✘ ≈ ✘ 北㓁) ]"]];
    for (i = 0, len = probes_and_matchers.length; i < len; i++) {
      ref1 = probes_and_matchers[i], probe = ref1[0], matcher = ref1[1];
      try {
        result = IDLX.parse(probe);
