...
(function() {
  ...
  this["(IDL) normalize formula"] = function(T) {
    var _shake_tree, ctx, glyph, glyphs_probes_and_matchers, i, len, matcher, probe, ref1, shake_tree;
    shake_tree = function(ctx) {
    ...
    _shake_tree = function(tree, delta) {
      var argument_idx, operator_symbol, operator_token, ref1, sub_operator_symbol, sub_operator_token, sub_tree, token_type;
      if ((CND.type_of(tree)) !== 'list') {
        throw new Error("### MEH 1 ###");
      }
      operator_token = tree[0];
      if ((CND.type_of(operator_token)) !== 'MOJIKURA-IDL/token') {
        throw new Error("### MEH 2 ###");
      }
      if (operator_token.t !== 'operator') {
      ...
      while (true) {
        ...
        if ((CND.type_of(sub_operator_token)) !== 'MOJIKURA-IDL/token') {
          throw new Error("### MEH 4 ###");
        }
        if (sub_operator_token.t !== 'operator') {
          throw new Error("### MEH 5 ###");
        }
        sub_operator_symbol = sub_operator_token.s;
        if (operator_symbol === sub_operator_symbol) {
          [].splice.apply(tree, [argument_idx, argument_idx - argument_idx + 1].concat(ref1 = sub_tree.slice(1))), ref1;
          argument_idx += -1;
        } else {
    ...
    glyphs_probes_and_matchers = [['㒚', '⿰亻⿱(⿱爫工彐)心'], ['㒚', '⿰亻⿱爫⿱工⿱彐心'], ['㒚', '⿰亻⿱⿱爫⿱工彐心'], ['㒚', '⿰亻⿱⿱⿱爫工彐心']];
