...
(function() {
  ...
  this["(IDL) _tokentree_as_text"] = function(T) {
    ...
    for (i = 0, len = probes_and_matchers.length; i < len; i++) {
      ...
      result = resume_next(T, function() {
        return IDL._tokentree_as_text(null, tokentree);
      });
      urge(JSON.stringify([probe, result]));
      T.eq(result, matcher);
    }
  ...
  if (module.parent == null) {
    include = ["sanity checks (private methods)", "sanity checks (grammar data)", "sanity checks (MKNCR)", "(IDL) parse simple formulas", "(IDL) reject bogus formulas", "(IDL) parse tree of simple formulas", "(IDL) _tokentree_as_text", "(IDLX) reject bogus formulas", "(IDLX) reject IDL operators with arity 3", "(IDLX) parse simple formulas", "(IDLX) parse extended formulas (plain)", "(IDLX) parse extended formulas (bracketed)", "(IDLX) reject bogus formulas (bracketed)", "(IDLX) reject bogus formulas (solitaires)"];
    this._prune();
    this._main();
    debug(IDLX.parse('⿺走日'));
