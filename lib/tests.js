...
(function() {
  ...
  this["reject bogus formulas"] = function(T) {
    ...
    probes_and_matchers = [[42, "expected a text, got a number"], ["", "syntax error (empty text)"], ["⿱⿰亻式⿱目八木木木", "syntax error @ token idx 7 of '⿱⿰亻式⿱目八木木木'"], ["⿺廴聿123", "syntax error @ token idx 3 of '⿺廴聿123'"], ["⿺", "syntax error (premature end of source '⿺'"], ["⿺⿺⿺⿺", "syntax error (premature end of source '⿺⿺⿺⿺'"]];
    for (i = 0, len = probes_and_matchers.length; i < len; i++) {
      ref = probes_and_matchers[i], probe = ref[0], matcher = ref[1];
      try {
        result = IDL.parse(probe);
