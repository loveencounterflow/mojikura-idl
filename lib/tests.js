...
(function() {
  ...
  this["(IDLX) reject bogus formulas (bracketed)"] = function(T) {
    var error, error1, i, len, matcher, message, probe, probes_and_matchers, ref1, result;
    probes_and_matchers = [["(⿰亻聿)", "syntax error: too few constituents in [ (⿰亻聿 ✘ ) ✘  ]"], ["(⿰亻)", "syntax error: too few constituents in [ (⿰亻 ✘ ) ✘  ]"], ["(⿰亻)聿", "syntax error: too few constituents in [ (⿰亻 ✘ ) ✘ 聿 ]"]];
    for (i = 0, len = probes_and_matchers.length; i < len; i++) {
      ref1 = probes_and_matchers[i], probe = ref1[0], matcher = ref1[1];
      try {
        result = IDLX.parse(probe);
  ...
  if (module.parent == null) {
    include = ["sanity checks (private methods)", "sanity checks (grammar data)", "sanity checks (MKNCR)", "(IDL) parse simple formulas", "(IDL) reject bogus formulas", "(IDL) parse tree of simple formulas", "(IDLX) reject bogus formulas", "(IDLX) reject IDL operators with arity 3", "(IDLX) parse simple formulas", "(IDLX) parse extended formulas (plain)", "(IDLX) parse extended formulas (bracketed)", "(IDLX) reject bogus formulas (bracketed)"];
    this._prune();
    this._main();
    info(IDLX.parse('⿰亻聿'));
    info(IDLX.parse('(⿰亻聿式)'));
