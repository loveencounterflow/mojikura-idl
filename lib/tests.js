...
(function() {
  var CND, IDL, IDLX, alert, badge, debug, demo_errors, echo, help, include, info, log, nice_text_rpr, ref, rpr, test, urge, warn, whisper,
  ...
  this["(IDLX) reject bogus formulas"] = function(T) {
    var error, error1, i, len, matcher, message, probe, probes_and_matchers, ref1, result;
    probes_and_matchers = [["木", "syntax error: lone token of type 'component' in [  ✘ 木 ✘  ]"], [42, "expected a text, got a number"], ["", "syntax error: empty text"], ["⿱⿰亻式⿱目八木木木", "syntax error: extra token(s) in [ ⿱⿰亻式⿱目八 ✘ 木 ✘ 木木 ]"], ["⿺廴聿123", "syntax error: extra token(s) in [ ⿺廴聿 ✘ 1 ✘ 23 ]"], ["⿺", "syntax error: premature end of source in [  ✘ ⿺ ✘  ])"], ["⿺⿺⿺⿺", "syntax error: premature end of source in [ ⿺⿺⿺ ✘ ⿺ ✘  ])"], ["⿺12", "syntax error: illegal token '1' (type 'other') in [ ⿺ ✘ 1 ✘ 2 ]"], ["(⿰亻聿式)", "syntax error: illegal token '(' (type 'other') in [  ✘ ( ✘ ⿰亻聿式) ]"]];
    for (i = 0, len = probes_and_matchers.length; i < len; i++) {
      ref1 = probes_and_matchers[i], probe = ref1[0], matcher = ref1[1];
      try {
        result = IDLX.parse(probe);
  ...
  if (module.parent == null) {
    include = ["sanity checks (private methods)", "sanity checks (grammar data)", "(IDL) parse simple formulas", "(IDL) reject bogus formulas", "(IDL) parse tree of simple formulas", "(IDLX) reject bogus formulas", "(IDLX) reject IDL operators with arity 3", "(IDLX) parse simple formulas", "(IDLX) parse extended formulas (plain)"];
    this._prune();
    this._main();
